// ═══════════════════════════════════════════
//  NovaTEch BD — Router + App Shell v2.0
...
function buildPageDivs(role) {
  const allIds = new Set(['dash','sale','cust','stock','att','salary','exp','tasks','chat','replace','analytics','teams','profile']);
  return [...allIds].map(id => `<div class="page" id="page-${id}">
...
function buildDrawer(role) {
  const items = MORE_ITEMS[role] || MORE_ITEMS.manager || [];
